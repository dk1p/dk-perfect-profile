    ...
    // Typing Effect for Preloader Text
    const words = [
        "DINESH PERFECT PROFILE"
      ];
      ...
      function type() {
        if (currentWordIndex < words.length) {
          if (currentLetterIndex < words[currentWordIndex].length) {
            typedTextElement.innerHTML += words[currentWordIndex][currentLetterIndex];
            currentLetterIndex++;
            ...
            setTimeout(() => {
              ...
              typedTextElement.innerHTML = ""; // Clear text for the next word
              type(); // Start typing the next word
            }, 1000); // Pause before starting the next word
          }
        } else {
          // Start preloader animation after typing is done
          startPreloader();
        }
      }
  
      // Show Text Preloader First
    ...
    $(window).on("load", function () {
        ...
        setTimeout(function () {
            $(".loader-bg").fadeOut(800); // Fade out after the animation completes
        }, 1500); // Time matches the CSS animation duration
    });
